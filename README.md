...
**Easy access:** Hosted on Hugging Face - [kembaoak/CDC_Health_Data](https://huggingface.co/datasets/kembaoak/CDC_Health_Data) (notebook streams directly from here - no local file needed)
...
Missing values were filled using iterative low-rank SVD based on ISLR Ch. 12.5.2. The rank parameter M was tuned via held-out RMSE, with M=7 selected as optimal.
...
| k-NN (k=5) | 0.3386 |
| Matrix completion (M=7) | 0.4056 |
 
Matrix completion was chosen over k-NN because it fills missing entries using low-rank SVD structure, making the completed matrix directly consistent with the assumptions of PCA in the next step.
...
└── data /processed
      ...
      ├── places_county_completed.csv          
      ├── places_X_standardized.csv            
      ├── places_X_completed_standardized.csv  
      ├── places_pca_scores.csv                
      ├── places_pca_loadings.csv              
      ├── places_kmeans_labels.csv             
      ├── places_kmeans_cluster_means.csv
